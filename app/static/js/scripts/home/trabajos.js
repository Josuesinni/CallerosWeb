...
$(document).ready(function () {
    ...
    $(document).on("click", ".event-trabajo", function () {
        ...
        fetch('/api/trabajo/informacion_trabajo?id=' + idTrabajo).then(response => response.json()).then(data => {
            $(".modal-body #fecha_trabajo_u").val(new Date(data[6]).toISOString().substring(0, 10));
            $(".modal-body #clave_trabajo_u").val(data[0]);
            $(".modal-body #clave_cliente_u").val(data[1]);
            $(".modal-body #nombre_cliente_trabajo_u").val(data[2]);
            $(".modal-body #telefono_cliente_trabajo_u").val(data[3]);
            $(".modal-body #descripcion_trabajo_u").val(data[5]);
            $(".modal-body #datos_auto_trabajo_u").val(data[4]);
            $(".modal-body #pago_trabajo_u").val(data[8]);
        })
    });
});